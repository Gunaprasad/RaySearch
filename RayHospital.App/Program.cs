...
namespace RayHospital.App
{
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      IConsultationsManager consultationsManager = new ConsultationManager();

      // Read hard-coded list of patients and book a consultation for each patient
      DateTime startDate = DateTime.Today;
      foreach (var registration in PatientRegistrationModel.patientModelList)
      {
        var registrationDate = startDate.AddDays(registration.OffSetDays);
        var patient = new PatientRegistrationModel { Name = registration.Name, Condition = registration.Condition};
